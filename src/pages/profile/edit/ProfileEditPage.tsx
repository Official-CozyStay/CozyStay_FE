...
type TextInputProfileItemId = Exclude<
  ProfileItemId,
  'languages' | 'residence' | 'birthYear'
>;
...
const ProfileEditPage = () => {
  ...
  const [activeProfileModal, setActiveProfileModal] =
    useState<TextInputProfileItemId | null>(null);
  ...
  const [profileValues, setProfileValues] = useState<
    Record<TextInputProfileItemId, string>
  >({
    job: '',
    dreamDestination: '',
    funFact: '',
  ...
  // 프로필 항목 저장 핸들러
  const handleProfileValueSave = (
    id: TextInputProfileItemId,
    value: string,
  ) => {
        ...
        <MainContent>
          ...
          <Section>
            <SectionHeader>
              ...
              <SectionDescription>
                커뮤니티 신뢰 구축을 위해 프로필 정보가 호스트와 게스트에게
                공개되며, CozyStay 플랫폼 전반에도 프로필이 표시될 수 있습니다.{' '}
          ...
          <Section>
            ...
            <InterestTags>
              ...
              {Array.from({
                length: Math.max(1, 3 - selectedInterests.length),
              }).map((_, index) => (
                <AddInterestButton
                  key={`add-${index}`}
